 ...
 */
@Invariant({"name != null","race != null","life >= 0 && life <= 500","mana >= 0 && mana <= 500"})
...
public class Player {
     ...
     */
    @Requires({"target != this","this.isAlive()","target!= null"})
    @Ensures({"this.getXp() == old(this.getXp()) + ((20 + this.getStrength() - target.getEndurance()) * 2)"})
     ...
     */
    @Requires("damages > 0")
    @Ensures("getLife() == (old(getLife()) - damages) || getLife() == 0")
     ...
     */
    @Requires({"target != this","this.isAlive()","target!= null"})
    @Ensures({"this.getMana() == ( old(this.getMana()) - (5 * this.getIntelligence() - target.getEndurance()) ) "
    		+ "|| this.getMana() == old(getMana())",
    		"this.getXp() == (old(getXp()) + ( (5 * this.getIntelligence() - target.getEndurance() ) * 2 )) "
    		+ "|| this.getXp() == old(getXp())"})
